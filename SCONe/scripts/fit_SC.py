...
if not os.path.exists(path_bb):
	cmd = 'python ' + path_scripts + 'fit_bb_extinction.py --data "{2}" --out_path "{3}" --write True --plots {8} --name {0} --EBV_host {1} --z {4} --d_mpc {5} --dates "{6}" --t0 {7: .2f} --modify_BB_MSW {9}'.format(sn,ebv_host_init,path_data,path_bb,z,d_mpc,path_dates_sn,t0_init,plot_BB, modify_BB_MSW)
...
if write:
	if covar:
		resids_cov,resids,t_max = construct_covariance_MG_v2(data[cond],path_mat,path_key,dic_transmission=filter_transmission_fast,model_func = model_func,valid_inds = key_dic['ind_valid'])
	...
	if Rv_fit:
		Rv = 'fit'
	mean, quantiles,dresults = fit_func(data[cond],filter_transmission_fast,inv_cov = inv_cov,k34 = k34,Rv=Rv,sys_err =sys_err, LAW = LAW,plot_corner=False,priors =priors,rec_time_lims = rec_time_lims)
	...
	obj = model_func(R13,v85,fM,Menv,t0,k34,filter_transmission = filter_transmission_fast, ebv = ebv, Rv = Rv, LAW = LAW, distance = 3.0856e19)
		...
		if bb_plot:
			...
			if lc_plot:
				...
				for i in tqdm.tqdm(range(len(random))):
					...
					obj_rand = model_func(R13_rand,v85_rand,fM_rand,Menv_rand,t0_rand,k34,filter_transmission = filter_transmission_fast, ebv = ebv_rand, Rv = Rv_rand, LAW = LAW, distance = 3.0856e19)
					#obj_rand = model_SC(R13_rand,v85_rand,fM_rand,Menv_rand,t0_rand,k34,filter_transmission = filter_transmission_fast, ebv = ebv_rand, Rv = Rv_rand, LAW = LAW, distance = 3.0856e19)
					lab_d = {x:'' for x in lab_band.keys()}
					filt_list = np.unique(data['filter'])
					_,ax_lc = plot_lc_model_only(obj_rand,filt_list,obj_rand.t_down,obj_rand.t_up, c_band, lab_d,offset,validity = True, fig = fig_lc, ax = ax_lc,linewidth = 2, alpha = 0.8/len(random))
				...
				#obj_mean = model_func(R13_mean,v85_mean,fM_mean,Menv_mean,t0_mean,k34,filter_transmission = filter_transmission_fast, ebv = ebv_mean, Rv = Rv_mean, LAW = LAW, distance = 3.0856e19)
				#obj_mean = model_SC(R13_mean,v85_mean,fM_mean,Menv_mean,t0_mean,k34,filter_transmission = filter_transmission_fast, ebv = ebv_mean, Rv = Rv_mean, LAW = LAW, distance = 3.0856e19)
				_,ax_lc = plot_lc_model_only(obj_mean,filt_list,obj_mean.t_down,obj_mean.t_up, c_band, lab_d,offset,validity = True, fig = fig_lc, ax = ax_lc,linewidth = 2, alpha = 1,ls = '--')
				
				R13_std, v85_std, fM_std, Menv_std, t0_std, ebv_std = np.std(random_full[:,:6],axis = 0)
				if Rv_fit:
					Rv_std = np.std(random_full[:,6])
				str2 = r'$R_{13}=$' + '{0:.1f}'.format(R13_mean) + r'$\pm$' + '{0:.1f}'.format(R13_std)
				str2 = str2 + r', $v_{*,8.5}=$' + '{0:.1f}'.format(v85_mean) + r'$\pm$' + '{0:.1f}'.format(v85_std)
				str3 = r'$M_{env}=$' + '{0:.2f}'.format(Menv_mean) + r'$\pm$' + '{0:.2f}'.format(Menv_std)
				str3 = str3 + r', $E(B-V)=$' + '{0:.2f}'.format(ebv_mean) + r'$\pm$' + '{0:.2f}'.format(ebv_std)
				if Rv_fit:
					str3 = str3 + r', $R_V=$' + '{0:.1f}'.format(Rv_mean) + r'$\pm$' + '{0:.1f}'.format(Rv_std)
